...
const mealPlannerTranslations = {
  foodCatalog: {
    ...
    subtitle: {
      fr: "Ajoutez des aliments au plan",
      en: "Add foods to the plan",
    },
...
const defaultFoods = [
  ...
  {
    id: "poulet-grille",
    name: "Poulet grillé (sans peau)",
    groups: ["Protéines", "Proteins"],
    unitType: "g",
    kcalPer100: 165,
    protPer100: 31.0,
    carbsPer100: 0.0,
  ...
  {
    id: "legumes-verts",
    name: "Légumes verts (mélange)",
    groups: ["Légumes", "Vegetables"],
    unitType: "g",
    kcalPer100: 30,
    protPer100: 2.0,
    carbsPer100: 5.0,
...
const kcalFor = (item) => {
  ...
  if (item.unitType === "portion")
    return Math.max(
      0,
      Math.round((item.kcalPerPortion || 0) * (Number(item.qty) || 0))
    );
  return Math.max(
    0,
    Math.round(((item.kcalPer100 || 0) * (Number(item.qty) || 0)) / 100)
  );
};
...
const fmtMacros = (m) =>
  `P: ${m.p.toFixed(1)} g • G: ${m.c.toFixed(1)} g • L: ${m.f.toFixed(1)} g`;
const sumKcal = (list) => Math.round(list.reduce((n, i) => n + kcalFor(i), 0));
const sumDay = (d) =>
  sumKcal(d.petit) + sumKcal(d.dej) + sumKcal(d.diner) + sumKcal(d.snack);

const Toast = ({ message, show, onHide }) => (
  <div
    className={`fixed right-4 bottom-4 bg-gray-800 dark:bg-gray-200 text-white dark:text-gray-800 px-4 py-2 rounded-lg transition-all duration-200 shadow-lg ${
      show ? "opacity-100 transform-none" : "opacity-0 transform translate-y-2"
    }`}
  >
    {message}
  </div>
...
export default function MealPlanner() {
  ...
  const filteredFoods = db.foods.filter((food) => {
    ...
    const matchGroup =
      ...
      food.groups.some((group) => {
        ...
        if (
          Object.entries(groupTranslations).find(
            ([fr, en]) =>
  ...
  const removeFoodFromMeal = (mealType, entryId) => {
    setDb((prev) => ({
      ...
      week: {
        ...
        [prev.day]: {
          ...prev.week[prev.day],
          [mealType]: prev.week[prev.day][mealType].filter(
  ...
  const dayTotals = {
    petit: {
      kcal: sumKcal(currentDay.petit),
      macros: sumMacros(currentDay.petit),
    },
    dej: { kcal: sumKcal(currentDay.dej), macros: sumMacros(currentDay.dej) },
    diner: {
      kcal: sumKcal(currentDay.diner),
      macros: sumMacros(currentDay.diner),
    },
    snack: {
      kcal: sumKcal(currentDay.snack),
      macros: sumMacros(currentDay.snack),
    },
  };

  const totalDayKcal = sumDay(currentDay);
  const totalWeekKcal = Object.keys(db.week).reduce(
    (total, day) => total + sumDay(db.week[day]),
    0
  );
  ...
  const generateDiet = () => {
    ...
    const mPetit = sumMacros(d.petit),
      mDej = sumMacros(d.dej);
    const mDiner = sumMacros(d.diner),
      mSnack = sumMacros(d.snack);
    const cur = {
      p: mPetit.p + mDej.p + mDiner.p + mSnack.p,
      c: mPetit.c + mDej.c + mDiner.c + mSnack.c,
      f: mPetit.f + mDej.f + mSnack.f + mDiner.f,
    };
    ...
    ["petit", "dej", "diner", "snack"].forEach((meal) => {
      newWeek[db.day][meal] = newWeek[db.day][meal].map((item) => {
        ...
        return {
          ...
          qty: Math.max(
            0,
            Math.round(((Number(item.qty) || 0) * sItem) / 5) * 5
          ),
        };
      });
    });
      ...
      >
        ...
        <div
          className={`p-2 min-h-[100px] ${
            !readOnly ? "bg-gray-50/50 dark:bg-gray-700/50" : ""
          }`}
        >
          {entries.length === 0 ? (
            ...
            entries.map((entry) => (
              <div
                key={entry.id}
                className="p-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded-lg my-2"
              >
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        handleInputChange(entry.id, e.target.value)
                      }
  ...
  const AddFoodModal = () => {
    ...
    const modalFilteredFoods = db.foods.filter((food) => {
      const matchText =
        food.name.toLowerCase().includes(modalSearch.toLowerCase()) ||
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl">
          <div className="flex items-center justify-between p-4 border-b border-gray-300 dark:border-gray-600">
            <strong className="text-gray-900 dark:text-gray-100">
              Ajouter un aliment — {mealNames[addTargetMeal]}
            </strong>
          ...
          <div className="p-4">
            ...
            <div className="max-h-96 overflow-auto border border-gray-300 dark:border-gray-600 rounded-xl">
              {modalFilteredFoods.map((food) => {
                const unitLabel =
                  food.unitType === "portion" ? "portion" : food.unitType;
                const kcalBaseNum =
                  food.unitType === "portion"
                    ? food.kcalPerPortion ??
                      Math.round(
                        (food.protPerPortion || 0) * 4 +
                          (food.carbsPerPortion || 0) * 4 +
                          (food.fatPerPortion || 0) * 9
                      )
                    : food.kcalPer100 ??
                      Math.round(
                        (food.protPer100 || 0) * 4 +
                          (food.carbsPer100 || 0) * 4 +
                          (food.fatPer100 || 0) * 9
                      );
                const kcalLabel =
                  food.unitType === "portion"
                    ? `${kcalBaseNum} kcal/portion`
                    : `${kcalBaseNum} kcal/100${unitLabel}`;

                return (
                  <div
                    key={food.id}
                    className="flex justify-between items-center gap-3 p-3 border-b border-gray-300 dark:border-gray-600 last:border-b-0 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
                  >
                    <div>
                  ...
                  {currentPatients.map((patient) => (
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                        ...
                        {t("patientSelection.modal.age")}
                        {patient.height && ` • ${patient.height}cm`}
                        {patient.latestVisit?.weight &&
                          ` • ${patient.latestVisit.weight}kg`}
                      </div>
                    </button>
                  ))}
                </div>
  ...
  const loadMealPlan = async (patient) => {
    try {
      ...
      const doctorName =
        doctorResponse.data.firstname + " " + doctorResponse.data.lastname;
      ...
      const targetKcal =
        patient.latestVisit?.calorieintake || computeTargetKcal();
            ...
            const foodItems = items
              ...
              .map((item) => {
                ...
                const match = item.match(
                  /(.*?)\s*\((\d+(?:\.\d+)?)\s*(g|ml|unité|portion|c\.c\.|c\.s\.)\)?/i
                );
                ...
                const food = defaultFoods.find(
                  (f) => f.name.toLowerCase().trim() === cleanName.toLowerCase()
                );
      ...
      setToast({
        show: true,
        message: `Patient ${patient.firstname} ${patient.lastname} sélectionné${
          response.data.dietPlan ? " (régime existant chargé)" : ""
        }`,
      });
      setTimeout(() => setToast((prev) => ({ ...prev, show: false })), 1200);
    } catch (error) {
  ...
  const saveDietPlan = async () => {
    try {
      ...
      const formatMealItems = (items) => {
        return items
          .map(
            (item) =>
              `${item.name} (${item.qty}${
                item.unitType === "portion" ? "unité" : item.unitType
              })`
          )
          .join(" + ");
      };
      ...
      const meals = {
        breakfast: ["J1", "J2", "J3", "J4", "J5", "J6", "J7"]
          .map((day) => formatDayMeals(day, "petit"))
          .filter(Boolean)
          .join("\n\n"),

        lunch: ["J1", "J2", "J3", "J4", "J5", "J6", "J7"]
          .map((day) => formatDayMeals(day, "dej"))
          .filter(Boolean)
          .join("\n\n"),

        dinner: ["J1", "J2", "J3", "J4", "J5", "J6", "J7"]
          .map((day) => formatDayMeals(day, "diner"))
          .filter(Boolean)
          .join("\n\n"),

        morningSnack: ["J1", "J2", "J3", "J4", "J5", "J6", "J7"]
          .map((day) => {
            const snacks = db.week[day].snack;
            if (snacks.length === 0) return "";
            return `${day}: ${formatMealItems(
              snacks.slice(0, Math.ceil(snacks.length / 2))
            )}`;
          })
          .filter(Boolean)
          .join("\n\n"),

        afternoonSnack: ["J1", "J2", "J3", "J4", "J5", "J6", "J7"]
          .map((day) => {
            const snacks = db.week[day].snack;
            if (snacks.length === 0) return "";
            return `${day}: ${formatMealItems(
              snacks.slice(Math.ceil(snacks.length / 2))
            )}`;
          })
          .filter(Boolean)
          .join("\n\n"),
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setNewFood((prev) => ({ ...prev, unitType: e.target.value }))
                }
                className="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2"
              >
              ...
              {filteredFoods.map((food) => (
                ...
                >
                  ...
                  >
                    ...
                    <option value="petit">
                      {t("foodCatalog.meals.breakfast")}
                    </option>
                    <option value="dej">{t("foodCatalog.meals.lunch")}</option>
                    <option value="diner">
                      {t("foodCatalog.meals.dinner")}
                    </option>
                    <option value="snack">
                      {t("foodCatalog.meals.snacks")}
                    </option>
                  </select>
                </div>
              ))}
          ...
          <div className="p-4">
              ...
              <input
                ...
                onChange={(e) =>
                  setDb((prev) => ({
                    ...
                    config: {
                      ...prev.config,
                      targetKcal: Number(e.target.value) || 0,
                    },
                  }))
                }
            ...
            <div className="bg-primary/5 dark:bg-primary/10 rounded-lg p-3 border border-primary/20 dark:border-primary/20">
              <div className="font-bold text-lg text-gray-900 dark:text-white">
                {t("dailyTotal.title")}: {totalDayKcal}{" "}
                {t("dailyTotal.calories")}
              </div>
              <div className="text-sm text-gray-700 dark:text-gray-300">
                {fmtMacros({
                  p:
                    dayTotals.petit.macros.p +
                    dayTotals.dej.macros.p +
                    dayTotals.diner.macros.p +
                    dayTotals.snack.macros.p,
                  c:
                    dayTotals.petit.macros.c +
                    dayTotals.dej.macros.c +
                    dayTotals.diner.macros.c +
                    dayTotals.snack.macros.c,
                  f:
                    dayTotals.petit.macros.f +
                    dayTotals.dej.macros.f +
                    dayTotals.diner.macros.f +
                    dayTotals.snack.macros.f,
                })}
              </div>
            </div>
          </div>
        ...
        <div id="nutrition-program" className="bg-white dark:bg-gray-800 p-6">
          ...
          <div className="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-5 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {[
                {
                  label: t("nutritionProgram.patientInfo.name"),
                  value: patient.nom,
                },
                {
                  label: t("nutritionProgram.patientInfo.age"),
                  value: `${patient.age} ${t(
                    "nutritionProgram.patientInfo.years"
                  )}`,
                },
                {
                  label: t("nutritionProgram.patientInfo.doctor"),
                  value: `Dr. ${patient.prat}`,
                },
                {
                  label: t("nutritionProgram.patientInfo.weight"),
                  value: `${patient.poids} kg`,
                },
                {
                  label: t("nutritionProgram.patientInfo.height"),
                  value: `${patient.taille} cm`,
                },
                {
                  label: t("nutritionProgram.patientInfo.bmi"),
                  value: calcIMC(),
                },
                {
                  label: t("nutritionProgram.patientInfo.bodyFat"),
                  value: `${patient.latestVisit?.bf || "-"} %`,
                },
                {
                  label: t("nutritionProgram.patientInfo.dailyCalories"),
                  value: `${db.config.targetKcal} kcal`,
                },
                {
                  label: t("nutritionProgram.patientInfo.date"),
                  value: patient.date,
                },
              ].map((field, index) => (
                <div
                      ...
                      >
                        ...
                        <td className="p-4 text-gray-700 dark:text-gray-300 border-r border-gray-100 dark:border-gray-700">
                          <div className="text-sm leading-relaxed">
                            {formatItems(day.petit)}
                          </div>
                        </td>
                        <td className="p-4 text-gray-700 dark:text-gray-300 border-r border-gray-100 dark:border-gray-700">
                          <div className="text-sm leading-relaxed">
                            {formatItems(day.snack)}
                          </div>
                        </td>
                        <td className="p-4 text-gray-700 dark:text-gray-300 border-r border-gray-100 dark:border-gray-700">
                          <div className="text-sm leading-relaxed">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-blue-800 dark:text-blue-200">
                  {["water", "chewing", "processed", "sugaryDrinks"].map(
                    (tip) => (
